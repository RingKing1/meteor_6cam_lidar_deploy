#!/usr/bin/env python3
"""Real-TensorRT INT8 lane probe on an x86 GPU (TensorRT 10, cuda-python).

ADAPTED FOR THE 6-CAMERA METEOR RIG (2026-09-23). Upstream origin:
  tier4/METEOR, branch `vla-overlay-int8-probes`, file `deploy/int8_lane_x86.py`
  (Apache-2.0; see ../meteor_train/LICENSE-METEOR-Apache-2.0).
Changes vs upstream are marked with `# [6cam]`:
  - cameras read from each scene's manifest (6-camera rigs have no NARROW slots);
  - calibration device buffers sized by --n-cams (was hard-coded 8);
  - scene root / val list default to this repo's layout (was /data/dataset/bevlane + ~/work/BevLane/val.lst).

Why this tool exists: on the Orin the INT8 engine of the 2:4-sparse model loses 23-56 % of its
laneline pixels vs fp16 while the dense model keeps them; a PyTorch fake-quant proxy did not
reproduce it. This builds fp16 / INT8 (/ INT8+sparse) engines from the PLAIN ONNX with an entropy
calibrator fed with real frames, then counts BEV lane-class pixels per engine on val frames
(the same measurement made on the Orin: pixel ratio int8/fp16 and per-class agreement).

  python3 patches/meteor_deploy/int8_lane_x86.py build --onnx out/meteor_v157.onnx --out engine/v157_fp16.plan
  python3 patches/meteor_deploy/int8_lane_x86.py build --onnx ... --out engine/v157_int8.plan \
      --int8 --fp16-keep layer1          # see README: layer1 must stay fp16 under INT8+sparse
  python3 patches/meteor_deploy/int8_lane_x86.py probe --ref engine/v157_fp16.plan --eng engine/v157_int8.plan

NOTE: `probe` imports `deploy.runtime.MeteorRT` -> run from a METEOR tree on PYTHONPATH.
"""
...
# [6cam] this repo's layout (override on the CLI / env)
B = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
ROOT = os.environ.get("METEOR_SCENES_ROOT", os.path.join(B, "meteor_6cam_lidar_deploy", "scenes"))
LIST = os.environ.get("METEOR_VAL_LIST", os.path.join(B, "meteor_6cam_lidar_deploy", "val_scenes.txt"))
...
def scene_names(list_path, root, n=0):
    """[6cam] scene names from a list file, else every scene dir under root."""
...
def frames(scenes, root, stride, per_scene, start=3):
    for s in scenes:
...
def build(a):
    ...
    if a.int8:
        ...
        scenes = scene_names(a.val_list, a.scenes_root)[a.n_eval:a.n_eval + 8]
        it = frames(scenes, a.scenes_root, 4, a.calib // 8 + 1)
        ...
        class Cal(trt.IInt8EntropyCalibrator2):
            def __init__(s2):
                ...
                for nm, nbytes in (("imgs", _N * 3 * 432 * 768), ("K", _N * 9 * 4),
                                   ("T_cam_ego", _N * 16 * 4), ("v0", 4)):
...
def probe(a):
    ...
    scenes = scene_names(a.val_list, a.scenes_root, a.n_eval)
    cnt = {k: [0, 0] for k in NAMES}; inter = {k: 0 for k in NAMES}; union = {k: 0 for k in NAMES}; n = 0; agree = 0; tot = 0
    for s, fi, imgs, K, T, v0 in frames(scenes, a.scenes_root, a.stride, 0):
...
if __name__ == "__main__":
    ...
    b.add_argument("--scenes-root", default=ROOT, help="[6cam] scene tree root")
    b.add_argument("--val-list", default=LIST, help="[6cam] scene-name list")
    ...
    p.add_argument("--scenes-root", default=ROOT, help="[6cam] scene tree root")
    p.add_argument("--val-list", default=LIST, help="[6cam] scene-name list")
